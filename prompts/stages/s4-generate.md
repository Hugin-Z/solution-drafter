...
- 当前 section 的 `section_prompt`（section 专属 anchor + 生成思路 + 输出格式）
...
**逐 section 落实（与 section prompt 配合）**：每个 section prompt 末尾有「## 素材运用（S2 / assets.json）」小节，列出该 section 必须消费的 acquired 素材步骤。执行该 section 的生成思路时，按其「素材运用」小节逐条把 acquired 素材织入论点 / 待补充落「【待补充】」。**不允许只跟生成思路填 intake 字段而跳过素材运用步骤**（素材依赖下沉到每个 section 的目的）。
