# -*- coding: utf-8 -*-
"""
docx_builder.py · 方案文档 docx 构建工具模块
...
参数(命令行模式):
    --out      : 输出 docx 路径,默认 output/solution_document.docx
    --project  : 项目名称(写到封面)
    --org      : 编制单位名称(写到封面)
    --date     : 日期(默认今天)
...
def apply_default_styles(doc: Document, body_font: str = "宋体"):
    """
    ...
    for style_name, conf in DEFAULT_STYLES.items():
        ...
        if rfonts is None:
            rfonts = OxmlElement("w:rFonts")
            rpr.insert(0, rfonts)
        east_asia = body_font if style_name == "Normal" else conf["east_asia"]
        rfonts.set(qn("w:eastAsia"), east_asia)
...
def add_paragraph(doc: Document, text: str,
                  ...
                  line_spacing=1.5, body_font: str = "宋体"):
    """
...
def _add_placeholder_box(doc: Document, text: str = "【图片占位】", body_font: str = "宋体"):
    """
    ...
    run = p.add_run(text)
    set_run_font(run, east_asia=body_font, size_pt=10.5, bold=False,
                 color=RGBColor(0x80, 0x80, 0x80))
    return p


def add_figure_caption(doc: Document, caption_text: str,
                       with_placeholder: bool = True, body_font: str = "宋体"):
    """
    ...
    """
    if with_placeholder:
        _add_placeholder_box(doc, body_font=body_font)
    ...
    run1 = p.add_run("图 ")
    set_run_font(run1, east_asia=body_font, size_pt=10.5, bold=False)
    insert_seq_field(p, "图")
    run2 = p.add_run(f" {caption_text}")
    set_run_font(run2, east_asia=body_font, size_pt=10.5, bold=False)
    return p
...
def add_cover_page(doc: Document, project_name: str, org_name: str,
                   doc_date: str = ""):
    """
    ...
    r5 = p5.add_run(f"日  期:{doc_date or date.today().strftime('%Y 年 %m 月 %d 日')}")
...
def create_tender_doc(out_path: Path,
    ...
    add_cover_page(doc, project_name, org_name, doc_date)
    add_toc_page(doc)
...
def create_section_doc(out_path: Path, body_font: str = "宋体") -> Path:
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="方案文档 docx 构建工具(可命令行创建空骨架)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    parser.add_argument("--out", default="output/solution_document.docx",
                        help="输出 docx 路径")
    parser.add_argument("--project", default="【项目名称】",
                        help="项目名称(写到封面)")
    parser.add_argument("--org", default="【编制单位】",
                        help="编制单位名称(写到封面)")
    ...
    if args.section_only:
        ...
        print(f"[完成] 方案文档空骨架已生成:{out_path}")
        print()
        print("=" * 60)
        print("下一步:进入阶段 4(分章节撰写)")
        print("  - 每写一章,调模块 API append_chapter.append_markdown(doc, md, body_font) 追加到此骨架")
