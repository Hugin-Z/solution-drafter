...
- **强契约 / 弱实现（S2 边界）**：solution-drafter 已把资料获取抽象成 Skill 契约（`asset_needs` 声明 + `assets.json` 产出 + S4 强依赖），但**自身不实现检索引擎、不提供真实素材库 provider**；真实素材需接你环境的本地材料库 / RAG / ripgrep / SQL / 上传文档（手段无关）。契约弱实现不等于可以不获取素材——你要用环境里的任意手段主动获取。
...
- **操作**：**agent 自己读 section prompt 自己生成 markdown**（不调 stub / 不调外部 LLM API / Skill 使用者自身是 LLM agent）。本 section 必须使用 `assets.json` 该 section 的 acquired 素材 / 待补充处写"【待补充】"。每 section 严格按 prompt 的 生成思路 + 输出格式 + 红线复述。
- **后果链**：S2 没发动 → assets.json 全待补充 → 本阶段大面积【待补充】→ S5 暴露内容空。这逼 S2 真获取素材。
- **长文档不断流（逐 section 落盘 / 强制）**：S4 每生成完一个 section 立即**追加写入 `draft.md`（文本文件 / 落盘）**，再生成下一个（docx 的逐段渲染在 S5 / 见下"docx 工具调用三步"）；单个 section 若过长按其三级标题分批生成 + 分批写入 draft.md（详见 `prompts/stages/s4-generate.md` 的"逐 section 落盘"节）。不要把整本攒在上下文里最后一次性写（几万~几十万字会撞 token 上限被截断 / 逐段落盘后中途断已写的保住）。
...
final_docx = output_dir / outline_cfg["output"]["filename_pattern"].format(project_name=safe_name)
...
# 3. 字体合规验证（传 declared_font=body_font 额外校验实际正文字体 == font_policy 声明）
...
2. 加 `prompts/sections/<new_doc_type>/<NN-section>.md`（每 section 含 anchor + intake 字段依赖 + 生成思路 + 输出格式 + 「## 素材运用（S2 / assets.json）」小节 + 红线复述）。
...
5. **不内置检索 / 不提供真实素材库**：S2 不实现检索引擎，但必须用环境手段主动获取素材；不得以"调占位"为由跳过 S2。`scripts/assets_provider.py` **只定义 `AssetsProvider` 抽象接口 + 一个占位实现**，不提供真实素材库 provider；其中保留的 `CuratedLocalAssetsProvider` 仅作接口示例、**已下架**（`get_provider('curated_local')` 直接报错），勿当真实库用——真实 provider 按 `AssetsProvider` 接口自行实现。
